...
const Project1 = () => {
  ...
  // Function to fetch interior design project data from the backend
  const fetchInteriorProjects = async () => {
    try {
      ...
      if (JSON.stringify(projectData) !== storedProjects) {
        ...
        localStorage.setItem('interiorDesignProjects', JSON.stringify(projectData)); // Update localStorage
      }
  ...
  useEffect(() => {
    ...
    if (storedProjects) {
      ...
      fetchInteriorProjects(); // Fetch data from the API if not in localStorage
    }

    // Set up interval to check for updates every 5 minutes (300000ms)
    const interval = setInterval(fetchInteriorProjects, 300000);
